...
2. First, we must select which frequency range we are going to test. Bring up the menu by tapping anywhere on the screen. Select the "Stimulus" option. Next, select the "Start" tab and enter "300M" to correspond to 300MHz. Navigate back to the menu, "Stimulus", and then "Stop". Enter "500M" to correspond to 500MHz. This configures a sweep frequency range 300 MHz to 500 MHz. Here is what your menu should look like:
...
4. Proceed with SOLT calibration. Bring up the menu by tapping anywhere on the screen and then select "Calibrate". Select "Calibrate" one more time. Connect the oepn connector  to port 1 and select the menu option for "open". Wait for the menu to un-highlight. Disconnect the open connector and safely store it.
...
12. Navigate back to the main menu and select the "Display" option. From here, choose "Format". Finally, select "SWR".Your menu should look as follows:
